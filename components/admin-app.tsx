...
import { Activity, Building2, Map, Plus, Save, School, ShieldCheck, Users } from 'lucide-react';
...
function AdminRegions(){
  const regions=[...new Map(sppgs.map(s=>[`${s.province}-${s.region}`,[s.province,s.region,'Active']])).values()];
  return <><PageHeader title="Wilayah" subtitle="Hierarki wilayah yang digunakan pada filter dan monitoring."/><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{regions.map(([p,c,s])=><Card key={c}><div className="flex items-center gap-3"><div className="rounded-xl bg-brand-50 p-3 text-brand-700"><Map className="h-5 w-5"/></div><div><div className="font-extrabold">{c}</div><div className="text-xs text-slate-500">{p}</div></div><div className="ml-auto"><StatusBadge status={s}/></div></div></Card>)}</div></>;
}
